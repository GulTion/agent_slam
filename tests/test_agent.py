...
class TestResearcherNode:
    ...
    def test_researcher_returns_queries(self, mock_build_llm, base_state):
        ...
        mock_llm_with_tools.invoke.return_value = self._make_ai_message_with_tool_call(
            [
                "AI regulation effectiveness statistics 2024",
                "AI market failure examples",
            ]
        )
        ...
        assert (
            "AI regulation effectiveness statistics 2024" in result["research_queries"]
        )
    ...
    def test_researcher_fallback_on_missing_tool_call(self, mock_build_llm, base_state):
        ...
        assert (
            "AI regulation" in result["research_queries"][0]
            or "evidence" in result["research_queries"][0]
        )
        ...
        mock_tavily.return_value = [
            {
                ...
                "content": "AI regulation reduces bias by 30%.",
                "query": "AI regulation statistics",
            },
            {
                "url": "https://stanford.edu/ai",
                "content": "AI incidents increased 40% without oversight.",
                "query": "AI market failure",
            },
        ]
        base_state["research_queries"] = [
...
class TestDebaterNode:
    ...
    def test_debater_returns_under_3000_chars(self, mock_build_llm, base_state):
        ...
        from src.agent.graph import MAX_RESPONSE_CHARS, debater_node
    ...
    def test_debater_two_citations_present(self, mock_build_llm, base_state):
        ...
        base_state["research_context"] = (
            "URL: https://mit.edu/ai-bias\nURL: https://stanford.edu/ai-safety\n"
        )
        result = debater_node(base_state)

        citation_count = len(
            re.findall(r"\(source:\s*https?://\S+\)", result["final_argument"])
        )
...
class TestFullPipeline:
    ...
    def test_end_to_end_pipeline(self, mock_build_llm, mock_tavily):
        ...
        mock_tavily.return_value = [
            {
                "url": "https://who.int/ai-safety",
                "content": "WHO recommends mandatory AI audits.",
                "query": "AI safety",
            },
            {
                "url": "https://ec.europa.eu/ai-act",
                "content": "EU AI Act passed with bipartisan support.",
        ...
        researcher_resp.tool_calls = [
            {
                "name": "tavily_search",
                "args": {"queries": ["AI safety WHO", "EU AI Act impact"]},
                "id": "c1",
            }
        ]
